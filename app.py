...
if uploaded_file is not None:
    ...
    # ── Per-sample: chart + inline prediction card ──
    for i in range(len(probs)):
        ...
        for j, row in df_prob.iterrows():
            fig.add_trace(go.Bar(
                ...
                textfont=dict(
                    family="DM Mono Bold, DM Mono, monospace",
                    size=22,
                    color="#f0c040",
                    weight=700,
                ),
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                showgrid=False,
                zeroline=False,
                tickfont=dict(family="DM Mono, monospace", size=16, color="#c9d0dc"),
                linecolor="#1e2d45",
            ),
            yaxis=dict(
                showgrid=True,
                gridcolor="rgba(30,45,69,0.7)",
                gridwidth=1,
                zeroline=False,
                tickformat=".0%",
                tickfont=dict(family="DM Mono, monospace", size=14, color="#6b7a96"),
                range=[0, 1.28],
                linecolor="#1e2d45",
            ),
            bargroupgap=0.25,
        ...
        # ── Inline prediction card directly under the chart ──
        ...
        <div style="
            ...
            gap: 32px;
        ">
          <div style="position:absolute;top:0;left:0;right:0;height:2px;background:{bar_gradient};"></div>

          <!-- Left: sample + class name -->
          <div>
            <div style="font-family:'DM Mono',monospace;font-size:11px;letter-spacing:0.2em;
                        color:#4a5568;text-transform:uppercase;margin-bottom:8px;">
              Prediction · Sample {i+1:02d}
            </div>
            <div style="font-family:'Cormorant Garamond',serif;font-size:42px;font-weight:700;
                        color:{text_color};letter-spacing:0.04em;line-height:1;margin-bottom:6px;">
              {cls}
            </div>
            <div style="font-family:'DM Sans',sans-serif;font-size:17px;font-weight:400;
                        color:#b0bcd4;line-height:1.4;">
              {full_name}
            </div>
          </div>

          <!-- Right: probability bar + value -->
          <div style="min-width:260px;flex-shrink:0;">
            ...
            <div style="background:rgba(255,255,255,0.06);border-radius:100px;
              ...
              <div style="width:{prob_pct}%;height:100%;border-radius:100px;
                          background:{bar_gradient};"></div>
            </div>
            <div style="font-family:'Cormorant Garamond',serif;font-size:32px;font-weight:700;
                        color:{accent_color};letter-spacing:0.04em;text-align:right;">
              {probability:.4f}
            </div>
          </div>
        </div>
